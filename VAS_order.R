...
cat("\n=== NORMALITY OF RESIDUALS (correct ANOVA assumption) ===\n")

# Fit a linear model including subject as factor
lm_model_seq <- lm(change ~ sequence + factor(id), data = df_complete)

# Extract residuals
res_seq <- residuals(lm_model_seq)

# Shapiro-Wilk test
shapiro_res <- shapiro.test(res_seq)
print(shapiro_res)
...
if (requireNamespace("ez", quietly = TRUE)) {
  library(ez)
  aov_order <- ezANOVA(
    data = df_complete %>% mutate(id = factor(id), sequence = factor(sequence)),
    dv = change,
    wid = id,
    within = sequence,
    type = 3,
...
friedman_matrix <- df_complete %>%
  select(id, sequence, change) %>%
  pivot_wider(names_from = sequence, values_from = change) %>%
  select(-id) %>%
  as.matrix()
...
orders <- sort(unique(df_complete$sequence))
pairs <- combn(orders, 2, simplify = FALSE)
...
pairwise_results <- lapply(pairs, function(pair) {
  ...
  data_wide <- df_complete %>%
    filter(sequence %in% pair) %>%
    select(id, sequence, change) %>%
    pivot_wider(names_from = sequence, values_from = change)
  
  w_test <- wilcox.test(
    data_wide[[as.character(pair[1])]],
    data_wide[[as.character(pair[2])]],
    paired = TRUE,
